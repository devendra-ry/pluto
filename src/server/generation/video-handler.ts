...
import { assertRateLimit, videoRateLimiter } from '@/server/http/rate-limit';
...
export async function POST(req: Request) {
    ...
    return executeIdempotentRequest(
        ...
        const payload = {
            ...
            frames: toBoundedInteger(parsed.data.frames, DEFAULT_FRAMES, 21, 140),
            fps: toBoundedInteger(parsed.data.fps, DEFAULT_FPS, 1, 60),
            fast: typeof parsed.data.fast === 'boolean' ? parsed.data.fast : false,
            guidance_scale: toBoundedNumber(parsed.data.guidance_scale, DEFAULT_GUIDANCE_SCALE, 0, 20),
        ...
        if (!bytes || bytes.length === 0) {
            ...
            console.error('[videos] upstream request failed', {
                upstreamUrl: lastAttemptUrl,
                status: lastStatus,
                error: lastErrorText,
            });
            return jsonResponse({ error: 'Video provider request failed. Please try again.' }, mappedStatus);
        }
        ...
        if (uploadError) {
            console.error('[videos] failed to store generated video', uploadError);
            return jsonResponse({ error: 'Failed to store generated video' }, 500);
        }
        ...
        return jsonResponse({
            ...
            await generationLock.release();
                ...
                console.error('[videos] request processing failed', error);
                await recordAbuseSignal(user.id, 'video', 'processing-error');
                return jsonResponse({ error: 'Failed to generate video' }, 500);
            }
        }
    );
}
