...
import { assertJsonRequest, assertValidPostOrigin, parseJsonObjectRequest, requireUser, toJsonErrorResponse } from '@/server/http/api-security';
import { assertRateLimit, imageRateLimiter } from '@/server/http/rate-limit';
import { createClient } from '@/utils/supabase/server';
import { getText, normalizePotentialBase64, readStringOrFirst, wait } from './media-utils';
    ...
    return executeIdempotentRequest(
        ...
        if (isImageEditRequest) {
            ...
            try {
                ...
                console.error('[images] failed to read source attachment', error);
                return jsonResponse({ error: 'Failed to read image attachment' }, 400);
            }
            ...
            if (targetApiUrls.length === 0) {
                ...
                console.error('[images] no upstream URL configured', { model: selectedModel, envKey });
                return jsonResponse({ error: 'Image generation is not configured for the selected model.' }, 500);
            }
        ...
        if (!generated) {
            ...
            console.error('[images] upstream request failed', {
                operation: isImageEditRequest ? 'edit' : 'generate',
                model: selectedModel,
                attempt: lastAttemptLabel,
                upstreamUrl: lastAttemptUrl,
                status: lastStatus,
                error: lastErrorText,
            });
            return jsonResponse({ error: 'Image provider request failed. Please try again.' }, mappedStatus);
        }
        ...
        if (uploadError) {
            console.error('[images] failed to store generated image', uploadError);
            return jsonResponse({ error: 'Failed to store processed image' }, 500);
        }
        ...
        return jsonResponse({
            ...
            await generationLock.release();
                ...
                console.error('[images] request processing failed', error);
                await recordAbuseSignal(user.id, 'image', 'processing-error');
                return jsonResponse({ error: 'Failed to process image request' }, 500);
            }
        }
    );
}
